...
//        btn.setTitle("Send", for: .normal)
    ...
    @objc func handleSendButton (){
        ...
        let id = AuthManager.authStudent?.id ?? 0
        let para : Parameters = ["message" : ms.text!, "fromId" : id, "isTeacher" : false, "name": AuthManager.authStudent?.name ?? ""]
        Alamofire.request("\(API.Urls.message)/\(self.courseID)", method: .post, parameters: para).response { (res) in
    ...
    func setupView(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(handleKeyNotification), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(handleKeyNotification), name: UIResponder.keyboardWillHideNotification, object: nil)
        if data.count > 0{
            UIView.animate(withDuration: 0.5) {
                self.collectionView?.scrollToItem(at: IndexPath(item: self.data.count - 1, section: 0), at: UICollectionView.ScrollPosition.top, animated: true)
            }
        }
...
//        getData()
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.profileImageView.image = UIImage(named: "background")
        cell.profileImageSender.image = UIImage(named: "background")
        cell.profileImageView.contentMode = .scaleAspectFill
        cell.profileImageSender.contentMode = .scaleAspectFill
...
//        view.backgroundColor = UIColor(white: 0.95, alpha: 1)
    ...
    override func setupView() {
        ...
        addConstrainsWithFormat("H:|-8-[v0(1)]", profileImageView)
        addConstrainsWithFormat("V:[v0(0)]|", profileImageView)
        profileImageView.backgroundColor = UIColor.red
        
        addConstrainsWithFormat("H:[v0(1)]-8-|", profileImageSender)
        addConstrainsWithFormat("V:[v0(0)]|", profileImageSender)
...
extension ChatLogCollectionViewController: StoryboardSceneBased {
    static var sceneStoryboard = Storyboards.chatBox
}
